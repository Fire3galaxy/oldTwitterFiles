...
public class TwitterSearchOld {
	public TwitterSearchOld() {
	...
	public void searchForDoctors(ExcelMethods excelList) {
		...
		for (int i = 3; i < excelList.getFile().size() && i < 77; i++) {
...
//		publishTopHandles();
	...
	public void search(ProfileDr query, int pageLimit) { 
		try {
			...
			List<ResponseList<User> > searchResults = new LinkedList<ResponseList<User>>();
...
//					System.out.println('@' + uag.user.getScreenName() + " " + uag.user.getName());
					...
					if ( containsKeyword(uag.user.getScreenName(), uag.user.getName(), 
							uag.user.getDescription(), uag.grade, query.getMiddleName()) ) {
...
//				System.out.println("-----------------------------------------"); // FIXME: Delete
				...
				for (UserAndGrade uag : HasKeywordList) {
					...
					if (!uag.user.isProtected()) {
						ResponseList<Status> timeline = twitter.getUserTimeline(uag.user.getScreenName(), pageOne);
						
						for (Status tweet : timeline) {
							gradeText(tweet.getText(), uag.grade);
						}
					}
				}
	...
	static boolean containsName(String user, String fn, String ln) {
...
//		    	System.out.println(i + " Failed");
	...
	private boolean containsKeyword(String handle, String name, String description, IntHolder grade, String middleName) {
		...
		gradeText(n, grade); // checks names for keywords and middle name
		gradeText(d, grade); // checks description for keywords
...
//		System.out.println("Grade: " + grade.getNum()); // FIXME: delete
	...
	private void gradeShortKeyword(String s, IntHolder grade) {
	...
	private void gradeName(String name, String middleName, IntHolder grade) {
		if (name.length() > middleName.length()) {
...
//				System.out.println("Handle contains " + shortKeywords[i]);
	...
	private void gradeText(String s1, IntHolder grade) {
		String s2 = s1.toLowerCase();
		
		for (int i = 0; i < keywords.length; i++) {
			if (s2.contains(keywords[i])) {
				grade.addNum(keywordValues[i]);
//				System.out.println(s2 + " Keyword: " + keywords[i]); // FIXME: Delete				
			}
		}
		
		gradeShortKeyword(s2, grade);
